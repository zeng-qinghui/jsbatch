var RUNTIME = new function(){
	var monitor = null;
	var storageMonitor = null;
	var taskList = [];
	var taskIndex = 0;
	var isRunning = false;
	var storage = {};
	
	var updateStorage = function(restore){
		if(storageMonitor){
			if(restore){
				storage = JSON.parse(storageMonitor.getValue());
			}else{
				storageMonitor.setValue(JSON.stringify(storage));	
			}
		}
	};
	...
	var fn = {
		...
		setStorageMonitor:function(o){
			storageMonitor = o;
		},
		applayStroage:function(){
			updateStorage(true);
		}
	};
